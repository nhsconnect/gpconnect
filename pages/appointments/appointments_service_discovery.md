...
The "default" GP connect flow (as used in other capabilities) is shown in the diagram below, and includes the retrieval of the **Organisation Data Service (ODS) organisation code of the patient's registered practice** from Personal Demographics Service (PDS) in step 1 (<span style="color: yellow; background-color: silver;">in yellow</span>).  For more details on this flow please see [Spine integration illustrated](integration_illustrated.html).
...
This flow is suitable for Appointment Management consumer organisations that wish to book into the patient's registered practice, but where an appointment is required at a different practice, an alternate mechanism for determining the practice and its ODS organisation code is needed.
...
Where a GP Connect Appointment Management consumer organisation wishes to book into a GP practice that is not the patient's registered practice, a different mechanism to discover the practice and its ODS organisation code is required.
...
For example there may be a pre-configured pick list of organisations on the booking screen of the consumer application, allowing a user to choose which practice to book into.  The pick list may be configured by the consumer organisation or by the consumer system supplier in conjunction with the consumer organisation.

#### Directory of Services (DOS) - currently for Urgent and Emergency Care (UEC) consumers only ####

Appointment consumer applications that are used to book into practices in a wider context where local knowledge of service availability cannot be relied on (such as 111 call centres that receive calls from across England) will need an enhanced mechanism for determining an appropriate practice to book the patient into, based on criteria such as:
...
Please see the following pages from the Urgent and Emergency Care Appointment Booking specification for further information on integrating GP Connect Appointment UEC consumer applications with Directory of Services: 
