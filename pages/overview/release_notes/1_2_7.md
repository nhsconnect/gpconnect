...
{% include important.html content="This issue was detected in an 'extended hours hub' organisation where PDS tracing had not been enabled for the entire time the organisation was active and consequently a number of active patients were not PDS traced or synchronised.  It is not expected this logic will be frequently used in GP practices since PDS is generally well established and most if not all patients will have been traced on PDS (or indirectly via NHAIS)." %}

**Pages changed:**

- [Find a patient](foundations_use_case_find_a_patient.html#provider-system-find-patient-requirements)
  - Add provider system requirements
