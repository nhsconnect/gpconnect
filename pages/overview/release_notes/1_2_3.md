...
- After consideration and discussion with suppliers, it has been decided that `Bundle.entry.fullUrl` shall not be populated for all our capabilities.  The reasoning behind this is as follows:
  - Lack of use cases
  - Difficulty in populating (e.g. needing to determine the correct external server root)
  - Not able to populate for resources that doesn't have [read](https://www.hl7.org/fhir/STU3/http.html#read) endpoint (e.g. AllergyIntolerance, MedicationStatement, Slot, Schedule)
  - Issues arising in integration testing
