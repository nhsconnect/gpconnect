...
- Where GP Connect consumer applications are hosted or provisioned by another organisation, the ASID sent in the Ssp-From header must reflect the organisation from where the request originated, rather than the hosting organisation
...
- Update SDS queries used by a consumer to locate a provider's FHIR service root URL
  ...
  - Only provider systems hold GP Connect interaction IDs on their MHS record, this enables the new queries to deterministically return the providers endpoint
...
- Also added to the SSP pages of the Spine Core specification
